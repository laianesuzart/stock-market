...
import { countWeekdays } from '@/shared/utils'
...
const formSchema = z.object({
	...
	dateRange: z.custom<DateRange>().superRefine((val, ctx) => {
		...
		if (!val.to || countWeekdays(val.from, val.to) < 2) {
			ctx.addIssue({
				code: 'custom',
				message: 'O período deve incluir pelo menos 2 dias úteis',
			})
		}
	}),
})
											...
											<Popover>
												<PopoverTrigger asChild>
													<Button
														...
														className={cn(
															...
															isInvalid &&
																'text-destructive border-destructive ring-3 ring-destructive/20',
														)}
														disabled={isSubmitting}
													>
														<CalendarIcon />
												...
												<PopoverContent className="w-auto p-0" align="start">
													<Calendar
														mode="range"
														defaultMonth={
															field.state.value?.from || subDays(today, 1)
														}
